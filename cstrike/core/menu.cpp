...
void MENU::RenderMainWindow()
{
	...
	if (ImGuiWindow* pMainWindow = ImGui::FindWindowByName(CS_XOR("GameSex")); pMainWindow != nullptr)
	{
	...
	// render main window
	ImGui::Begin(CS_XOR("Gamesex - Developer mode"), &bMainWindowOpened, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoCollapse);
	{
		...
		const ImVec2 vecTitleSize = FONT::pMenu[C_GET(int, Vars.nDpiScale)]->CalcTextSizeA(12.f * flDpiScale, FLT_MAX, -1.f, CS_XOR("GameSex"));
		pDrawList->AddText(ImVec2(vecMenuPos.x + vecMenuSize.x - style.WindowPadding.x - vecTitleSize.x, vecMenuPos.y + style.WindowPadding.y), ImGui::GetColorU32(ImGuiCol_Text), CS_XOR("GameSex"));
...
void MENU::RenderOverlayPreviewWindow()
{
	...
	ImGui::Begin(CS_XOR("preview"), &bMainWindowOpened, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
	{
		...
		if (const auto& nameOverlayConfig = C_GET(TextOverlayVar_t, Vars.overlayName); nameOverlayConfig.bEnable)
			context.AddComponent(new CTextComponent(true, SIDE_TOP, DIR_TOP, FONT::pVisual, CS_XOR("GameSex"), Vars.overlayName));
...
void MENU::RenderWatermark()
{
	...
	ImGui::BeginMainMenuBar();
	{
		...
		ImGui::TextColored(ImVec4(1.0f, 1.0f, 0.0f, 1.0f), CS_XOR("Insecure mode "));
		if (I::Engine->IsInGame())
			ImGui::TextColored(ImVec4(1.0f, 0.0f, 0.0f, 1.0f), CS_XOR("in game ez 1 for gamesex"));

		static ImVec2 vecNameSize = ImGui::CalcTextSize(CS_XOR("GameSex | " __DATE__ " " __TIME__));
		ImGui::SameLine(ImGui::GetContentRegionMax().x - vecNameSize.x - style.FramePadding.x);
		ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), CS_XOR("GameSex | " __DATE__ " " __TIME__));
	}
...
void MENU::UpdateStyle(ImGuiStyle* pStyle)
{
	...
	style.AnimationSpeed = C_GET(float, Vars.flAnimationSpeed) / 13.f;
}
...
void T::Credits()
{
	ImGui::TextUnformatted(CS_XOR("Made by the best dev no paste ez 1"));
}
